...
function startP(province) {
 ...
 //设置分页符号
 function showPages() {
    var stores = coffee["milkTea"]                  //获取所有的商店
...
function showInfo() {
    ...
    var milkTeaUl = document.getElementById('milkTeaList')   //获取ul节点
    var strMilkTea = ''
    ...
    for (var i = currentPage * pageSize; i < (currentPage + 1) * pageSize && i < recordCount; i++) {
        strMilkTea += '<span><li><img src="' + stores[i].image_url + '"></img><h3><a href="../views/shoping.html">' + stores[i].name + '</br>'+ stores[i].price + '</a></h3><p>'  + stores[i].con + '</p></li></span>'
    }
    milkTeaUl.innerHTML = strMilkTea
    var ps = document.getElementById('milkTeaList').getElementsByTagName('p')
    var hs = document.getElementById('milkTeaList').getElementsByTagName('h3')
    var spans = document.getElementById('milkTeaList').getElementsByTagName('li')
    var newSpans = $.makeArray(spans)
    for (let i = 0; i < ps.length; i++) {
        newSpans[i].onmouseover = function () {
            ps[i].style.display = "block"
            hs[i].style.display = "block"
            newSpans[i].style.backgroundColor = '#ccc';
        }
