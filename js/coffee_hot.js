...
function weak() {
    var now = new Date()
    var sunday = new Date("2019-8-16 21:30:0")
    var time = sunday - now
    time = time / 1000
    ...
    var spans = document.getElementById('bigLeft').getElementsByTagName('span')
    var str = cDate(day) + cDate(hour) + cDate(minute) + cDate(second)
    for (var i = 0; i < spans.length; i++) {
        spans[i].innerHTML = str[i]
    }
    setInterval(weak, 1000)
}
//设置分页符号
function showPages() {
    var stores = coffee["highPriceCoffee"]                  //获取所有的商店
...
function showInfo() {
    ...
    var coffeeUl = document.getElementById('coffeeList')   //获取ul节点
    var strCoffee = ''
    ...
    for (var i = currentPage * pageSize; i < (currentPage + 1) * pageSize && i < recordCount; i++) {
        strCoffee += '<li><img src="' + stores[i].image_url + '"></img><h3>' + stores[i].name + '</br>' + stores[i].price + '</h3><p>' + stores[i].con + '</p></li>'
    }
    coffeeUl.innerHTML = strCoffee
    var ps = document.getElementById('coffeeList').getElementsByTagName('p')
    var hs = document.getElementById('coffeeList').getElementsByTagName('h3')
    var lis = document.getElementById('coffeeList').getElementsByTagName('li')
    var newlis = $.makeArray(lis)
    for (let i = 0; i < ps.length; i++) {
        newlis[i].onmouseover = function () {
            ps[i].style.display = "block"
            hs[i].style.display = "block"
            newlis[i].style.backgroundColor = '#ccc';
            newlis[i].style.border = '15px solid #ccc';
        }
        newlis[i].onmouseout = function () {
            ps[i].style.display = "none"
            hs[i].style.display = "none"
            newlis[i].style.backgroundColor = '#e7e7e7';
            newlis[i].style.border = "none"
        }

    }
}
function bargainShow() {
    var stores = coffee["highPriceCoffee"] //获取所有的商店
    recordCount = stores.length
    var bargainUl = document.getElementById('bargainList')   //获取ul节点
    var strCoffee = ''
    //利用循环遍历JSON数据，并且通过DOM动态添加到页面中
    //第1页：0-9；第2页：10-19;第3页：20-29  i<recordCount(判断如果最后一页内的信息个数如果小于pagesize，也可以生成一页（最后一页）) 
    for (var i = 1; i <= 2; i++) {
        strCoffee += '<li><img src="' + stores[i].image_url + '"></img></br><s>' + stores[i].name + stores[i].price + '</s></li>'
    }
    bargainUl.innerHTML = strCoffee
}
//显示指定页的页面数据
function show(index) {
